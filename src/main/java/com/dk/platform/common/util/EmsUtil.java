...
import com.dk.platform.common.ems.common.tibjmsPerfCommon;
...
public class EmsUtil extends tibjmsPerfCommon {
     ...
     * @throws TibjmsAdminException             :           {@link TibjmsAdminException}
     */
    public EmsUtil(String filePath) throws TibjmsAdminException {

        this.eppConf = new EppConf(filePath);
...
//            tibjmsAdmin = new TibjmsAdmin(AppPro.EMS_URL.getValue(), AppPro.EMS_USR.getValue(), AppPro.EMS_PWD.getValue());
            tibjmsAdmin = new TibjmsAdmin(eppConf.ems.getUrlVal(), eppConf.ems.getUsrVal(), eppConf.ems.getPwdVal());
    ...
    private void setEmsConnection() throws JMSException {
        ...
        if(connection == null){
            ...
            if(factory == null){
                factory = new TibjmsConnectionFactory(eppConf.ems.getUrlVal());
            }
            connection = factory.createConnection(eppConf.ems.getUsrVal(), eppConf.ems.getPwdVal());
            connection.start();
...
//            return tibjmsAdmin.getQueue(AppPro.EMS_MNG_QUEUE_NAME.getValue()).getName();
            return tibjmsAdmin.getQueue(eppConf.ems.getManagerQueueVal()).getName();
        }catch (Exception e){
